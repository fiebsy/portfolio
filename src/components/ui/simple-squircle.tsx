...
const generateSquirclePath = (
  ...
  const rTL =
    radiusTopLeft !== undefined ? Math.min(radiusTopLeft, maxRadius) : Math.min(radius, maxRadius);

  const rTR =
    radiusTopRight !== undefined
      ? Math.min(radiusTopRight, maxRadius)
      : Math.min(radius, maxRadius);

  const rBR =
    radiusBottomRight !== undefined
      ? Math.min(radiusBottomRight, maxRadius)
      : Math.min(radius, maxRadius);

  const rBL =
    radiusBottomLeft !== undefined
      ? Math.min(radiusBottomLeft, maxRadius)
      : Math.min(radius, maxRadius);
  ...
  (
    ...
    const updateDimensions = useCallback(() => {
      ...
      if (rect.width > 0 && rect.height > 0) {
        ...
        // Mark as ready after dimensions are measured
        setIsReady(true);
      }
    }, []);
    ...
    useEffect(() => {
      ...
      if (wrapperRef.current) {
        ...
        setTimeout(updateDimensions, 20);
    ...
    const outerSquirclePath =
      widthInPx > 0 && heightInPx > 0
    ...
    const innerSquirclePath =
      widthInPx > 0 && heightInPx > 0 && hasBorder && borderWidth > 0 && borderStyle === 'solid'
        ? generateSquirclePath(
            widthInPx - borderWidth * 2,
            heightInPx - borderWidth * 2,
            Math.max(0, finalBorderRadius - borderWidth),
            borderRadiusTopLeft !== undefined
              ? Math.max(0, borderRadiusTopLeft - borderWidth)
              : undefined,
            borderRadiusTopRight !== undefined
              ? Math.max(0, borderRadiusTopRight - borderWidth)
              : undefined,
            borderRadiusBottomRight !== undefined
              ? Math.max(0, borderRadiusBottomRight - borderWidth)
              : undefined,
            borderRadiusBottomLeft !== undefined
              ? Math.max(0, borderRadiusBottomLeft - borderWidth)
              : undefined,
            finalCornerSmoothing,
            finalPointsPerCorner
          )
