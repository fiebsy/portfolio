...
interface BadgeProps {
  ...
  roundnessLevel?: number; // Added for SimpleSquircle compatibility
  ...
  (
    {
      ...
      roundnessLevel, // Destructured to prevent passing to DOM
      url,
      ...props
    },
    ref
    ...
    const renderIcon = (icon: IconType | string | undefined, position: 'left' | 'right') => {
      ...
      if (typeof icon !== 'string') {
        ...
        return (
          <div
            className={`flex items-center justify-center ${iconSizeClasses[size]} ${iconOnly ? '' : position === 'left' ? 'mr-1' : 'ml-1'}`}
          >
